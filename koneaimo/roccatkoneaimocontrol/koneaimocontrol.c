...
static GOptionContext *commandline_parse(int *argc, char ***argv) {
	...
	string = g_strdup_printf(_("- controls extended capabilities of Roccat %s mice"), KONEAIMO_DEVICE_NAME);
...
static gboolean print_actual_profile(RoccatDevice *koneaimo, GError **error) {
	guint profile_index = koneplus_actual_profile_read(koneaimo, error);
	if (*error)
		return FALSE;
	g_print("%i\n", profile_index + 1);
	return TRUE;
...
static gboolean print_firmware(RoccatDevice *koneaimo, GError **error) {
	gchar *firmware_string;
	guint firmware_version;

	firmware_version = koneplus_firmware_version_read(koneaimo, error);
	if (*error)
		return FALSE;
...
static gboolean print_sensor_value(RoccatDevice *koneaimo, GError **error) {
	guint8 value;
	value = koneplus_sensor_read_register(koneaimo, parameter_sensor_register, error);
	if (*error)
		return FALSE;
	g_print("0x%02x\n", value);
	return TRUE;
...
static gboolean reset(RoccatDevice *koneaimo, GError **error) {
	DBusGProxy *proxy;
	guint i;

	if (!koneplus_reset(koneaimo, error))
		return FALSE;

	proxy = koneaimo_dbus_proxy_new();
	if (!proxy)
		return TRUE;

	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i)
		koneaimo_dbus_emit_profile_data_changed_outside(proxy, i + 1);
...
int main(int argc, char **argv) {
	...
	koneaimo = koneaimo_device_first();
	if (koneaimo == NULL) {
		g_critical(_("No %s found."), KONEAIMO_DEVICE_NAME);
		retval = EXIT_FAILURE;
		goto exit1;
	}

	if (parameter_just_activate_driver_state) {
		if (!koneplus_device_state_write(koneaimo, KONEPLUS_DEVICE_STATE_STATE_ON, &local_error)) {
	...
	if (parameter_just_deactivate_driver_state) {
		if (!koneplus_device_state_write(koneaimo, KONEPLUS_DEVICE_STATE_STATE_OFF, &local_error)) {
	...
	if (parameter_just_update_firmware) {
		if (!roccat_firmware_update_console(koneaimo, KONEPLUS_INTERFACE_MOUSE, parameter_just_update_firmware,
	...
	if (parameter_just_update_srom) {
		if (!roccat_srom_update_console(koneaimo, KONEPLUS_INTERFACE_MOUSE, parameter_just_update_srom)) {
			retval = EXIT_FAILURE;
		}
		goto exit2;
	}
	...
	if (parameter_just_print_actual_profile) {
		if (!print_actual_profile(koneaimo, &local_error)) {
	...
	if (parameter_read_firmware) {
		if (!print_firmware(koneaimo, &local_error)) {
	...
	if (parameter_sensor_read) {
		if (!print_sensor_value(koneaimo, &local_error)) {
	...
	if (parameter_sensor_write != -1) {
		koneplus_sensor_write_register(koneaimo, parameter_sensor_register, parameter_sensor_write, &local_error);
		if (local_error) {
	...
	if (parameter_load != -1) {
		rmp = koneaimo_rmp_load(koneaimo, parameter_load - 1, &local_error);
		if (!rmp) {
		...
		rmp = koneplus_rmp_read_with_path(parameter_in_rmp, koneaimo_rmp_defaults(), &local_error);
		if (!rmp) {
	...
	if (parameter_save != -1) {
		if (!koneaimo_rmp_save(koneaimo, rmp, parameter_save - 1, &local_error)) {
			g_critical(_("Could not save profile %i: %s"), parameter_save, local_error->message);
			retval = EXIT_FAILURE;
			goto exit3;
		}
		koneaimo_dbus_emit_profile_data_changed_outside_instant(parameter_save);
	}
	...
	if (parameter_activate_profile != -1) {
		if (!koneplus_actual_profile_write(koneaimo, parameter_activate_profile -1, &local_error)) {
			g_critical(_("Could not activate profile %i: %s"), parameter_activate_profile, local_error->message);
			retval = EXIT_FAILURE;
			goto exit3;
		}
		koneaimo_dbus_emit_profile_changed_outside_instant(parameter_activate_profile);
	}

exit3:
	koneplus_rmp_free(rmp);
exit2:
	g_object_unref(G_OBJECT(koneaimo));
exit1:
