...
GType koneaimo_eventhandler_get_type(void);

#define KONEAIMO_EVENTHANDLER_TYPE (koneaimo_eventhandler_get_type())
#define KONEAIMO_EVENTHANDLER(obj) (G_TYPE_CHECK_INSTANCE_CAST((obj), KONEAIMO_EVENTHANDLER_TYPE, KoneaimoEventhandler))
#define IS_KONEAIMO_EVENTHANDLER(obj) (G_TYPE_CHECK_INSTANCE_TYPE((obj), KONEAIMO_EVENTHANDLER_TYPE))
#define KONEAIMO_EVENTHANDLER_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), KONEAIMO_EVENTHANDLER_TYPE, KoneaimoEventhandlerClass))
#define IS_KONEAIMO_EVENTHANDLER_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), KONEAIMO_EVENTHANDLER_TYPE))
#define KONEAIMO_EVENTHANDLER_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), KONEAIMO_EVENTHANDLER_TYPE, KoneaimoEventhandlerPrivate))

typedef struct _KoneaimoEventhandler KoneaimoEventhandler;
typedef struct _KoneaimoEventhandlerClass KoneaimoEventhandlerClass;
typedef struct _KoneaimoEventhandlerPrivate KoneaimoEventhandlerPrivate;

struct _KoneaimoEventhandler {
	GObject parent;
	KoneaimoEventhandlerPrivate *priv;
};

struct _KoneaimoEventhandlerClass {
	GObjectClass parent_class;
};

struct _KoneaimoEventhandlerPrivate {
	RoccatEventhandlerHost *host;
	KoneaimoDBusServer *dbus_server;
	...
	KoneaimoGfx *gfx;
	KoneaimoEventhandlerChannel *channel;
...
static void profile_changed(KoneaimoEventhandler *eventhandler, guint profile_number) {
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	...
	koneaimo_dbus_server_emit_profile_changed(priv->dbus_server, profile_number);
}

static gboolean set_profile(KoneaimoEventhandler *eventhandler, guint profile_number) {
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	guint profile_index;
...
static void window_changed_cb(RoccatEventhandlerHost *host, gchar const *title, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	guint profile_index;
...
static gboolean talk_easyshift(KoneaimoEventhandler *eventhandler, gboolean state) {
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
...
static gboolean talk_easyshift_lock(KoneaimoEventhandler *eventhandler, gboolean state) {
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval = FALSE;
...
static gboolean talk_easyaim(KoneaimoEventhandler *eventhandler, guchar state) {
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval = FALSE;
...
static gboolean should_execute_fx(KoneaimoEventhandler *eventhandler) {
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	return koneplus_rmp_get_talkfx(priv->rmp[priv->actual_profile_index]) == KONEPLUS_PROFILE_SETTINGS_TALKFX_ON;
}

static gboolean talkfx_set_led_rgb(KoneaimoEventhandler *eventhandler, guint32 effect, guint32 ambient_color, guint32 event_color) {
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval;
...
static gboolean talkfx_restore_led_rgb(KoneaimoEventhandler *eventhandler) {
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	GError *error = NULL;
	gboolean retval;
...
static guint koneaimo_eventhandler_get_talk_device(RoccatEventhandlerPlugin *self) {
	KoneaimoEventhandlerPrivate *priv = KONEAIMO_EVENTHANDLER(self)->priv;
...
static void koneaimo_eventhandler_talk_easyshift(RoccatEventhandlerPlugin *self, gboolean state) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(self);
	talk_easyshift(eventhandler, state);
}

static void koneaimo_eventhandler_talk_easyshift_lock(RoccatEventhandlerPlugin *self, gboolean state) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(self);
	talk_easyshift_lock(eventhandler, state);
}

static void koneaimo_eventhandler_talk_easyaim(RoccatEventhandlerPlugin *self, guchar state) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(self);
	talk_easyaim(eventhandler, state);
}

static void koneaimo_eventhandler_talkfx_set_led_rgb(RoccatEventhandlerPlugin *self, guint32 effect, guint32 ambient_color, guint32 event_color) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(self);
	talkfx_set_led_rgb(eventhandler, effect, ambient_color, event_color);
}

static void koneaimo_eventhandler_talkfx_restore_led_rgb(RoccatEventhandlerPlugin *self) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(self);
	talkfx_restore_led_rgb(eventhandler);
}

static void talk_easyshift_cb(KoneaimoDBusServer *server, guchar state, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	talk_easyshift(eventhandler, state);
}

static void talk_easyshift_lock_cb(KoneaimoDBusServer *server, guchar state, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	talk_easyshift_lock(eventhandler, state);
}

static void talk_easyaim_cb(KoneaimoDBusServer *server, guchar state, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	talk_easyaim(eventhandler, state);
}

static void talkfx_set_led_rgb_cb(KoneaimoDBusServer *server, guint effect, guint ambient_color, guint event_color, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	talkfx_set_led_rgb(eventhandler, effect, ambient_color, event_color);
}

static void talkfx_restore_led_rgb_cb(KoneaimoDBusServer *server, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	talkfx_restore_led_rgb(eventhandler);
}

static void gfx_set_led_rgb_cb(KoneaimoDBusServer *server, guint index, guint color, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	if (!should_execute_fx(eventhandler))
		return;
	koneaimo_gfx_set_color(eventhandler->priv->gfx, index, color);
}

static void gfx_get_led_rgb_cb(KoneaimoDBusServer *server, guint index, guint *color, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	if (!should_execute_fx(eventhandler))
		return;
	*color = koneaimo_gfx_get_color(eventhandler->priv->gfx, index);
}

static void gfx_update_cb(KoneaimoDBusServer *server, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	if (!should_execute_fx(eventhandler))
		return;
	(void)koneaimo_gfx_update(eventhandler->priv->gfx, NULL);
}

static void open_gui_cb(gpointer source, gpointer user_data) {
	gchar *path;
	path = g_build_path("/", INSTALL_PREFIX, "bin", "roccatkoneaimoconfig", (gchar const *)NULL);
	roccat_double_fork(path);
	g_free(path);
}

static void profile_data_changed_outside_cb(KoneaimoDBusServer *server, guchar number, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	...
	priv->rmp[profile_index] = koneaimo_rmp_load_actual(profile_index);
}

static void configuration_reload(KoneaimoEventhandler *eventhandler) {
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	if (priv->config)
		koneplus_configuration_free(priv->config);
	priv->config = koneaimo_configuration_load();
}

static void configuration_changed_outside_cb(KoneaimoDBusServer *server, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	configuration_reload(eventhandler);
}

static void cpi_changed_cb(KoneaimoEventhandlerChannel *channel, guchar cpi_index, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	guint cpi_value;
...
static void quicklaunch_cb(KoneaimoEventhandlerChannel *channel, guchar key_index, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
...
static void sensitivity_changed_cb(KoneaimoEventhandlerChannel *channel, guchar x, guchar y, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
...
static void talk_cb(KoneaimoEventhandlerChannel *channel, guchar key_index, guchar event, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	KoneplusRmpMacroKeyInfo *macro_key_info;
...
static void timer_start_cb(KoneaimoEventhandlerChannel *channel, guchar key_index, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	KoneplusRmpMacroKeyInfo *macro_key_info;
...
static void timer_stop_cb(KoneaimoEventhandlerChannel *channel, gpointer user_data) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(user_data);
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
...
static gboolean profile_data_load(KoneaimoEventhandler *eventhandler, GError **error) {
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	guint i;

	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		priv->rmp[i] = koneaimo_rmp_load(priv->device, i, error);
		if (priv->rmp[i] == NULL)
			return FALSE;
	}
	return TRUE;
...
static void device_add_cb(RoccatDeviceScannerInterface *interface, RoccatDevice *device, gpointer user_data) {
	...
	if (!priv->profile_note)
		priv->profile_note = roccat_notification_profile_new(roccat_eventhandler_host_get_notificator(priv->host), KONEAIMO_DEVICE_NAME);
	if (!priv->cpi_note)
		priv->cpi_note = roccat_notification_cpi_new(roccat_eventhandler_host_get_notificator(priv->host), KONEAIMO_DEVICE_NAME);
	if (!priv->sensitivity_note)
		priv->sensitivity_note = roccat_notification_sensitivity_new(roccat_eventhandler_host_get_notificator(priv->host), KONEAIMO_DEVICE_NAME);
	if (!priv->timer_note)
		priv->timer_note = roccat_notification_timer_new(roccat_eventhandler_host_get_notificator(priv->host), KONEAIMO_DEVICE_NAME);
	...
	priv->gfx = koneaimo_gfx_new(priv->device);

	// FIXME create in init and just disconnect/reconnect?
	priv->dbus_server = koneaimo_dbus_server_new();
	...
	koneaimo_dbus_server_connect(priv->dbus_server);
	...
	koneaimo_eventhandler_channel_start(priv->channel, priv->device, &local_error);
	if (local_error) {
...
static void stop(KoneaimoEventhandler *eventhandler) {
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	guint i;
	...
	koneaimo_eventhandler_channel_stop(priv->channel);
...
static void koneaimo_eventhandler_start(RoccatEventhandlerPlugin *self, RoccatEventhandlerHost *host) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(self);
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
	...
	priv->channel = koneaimo_eventhandler_channel_new();
	...
	priv->device_scanner = koneaimo_device_scanner_new();
...
static void koneaimo_eventhandler_stop(RoccatEventhandlerPlugin *self) {
	KoneaimoEventhandler *eventhandler = KONEAIMO_EVENTHANDLER(self);
	KoneaimoEventhandlerPrivate *priv = eventhandler->priv;
...
static void koneaimo_roccat_eventhandler_interface_init(RoccatEventhandlerPluginInterface *iface) {
	iface->start = koneaimo_eventhandler_start;
	iface->stop = koneaimo_eventhandler_stop;
	iface->get_talk_device = koneaimo_eventhandler_get_talk_device;
	iface->talk_easyshift = koneaimo_eventhandler_talk_easyshift;
	iface->talk_easyshift_lock = koneaimo_eventhandler_talk_easyshift_lock;
	iface->talk_easyaim = koneaimo_eventhandler_talk_easyaim;
	iface->talkfx_set_led_rgb = koneaimo_eventhandler_talkfx_set_led_rgb;
	iface->talkfx_restore_led_rgb = koneaimo_eventhandler_talkfx_restore_led_rgb;
...
G_DEFINE_TYPE_WITH_CODE(KoneaimoEventhandler, koneaimo_eventhandler, G_TYPE_OBJECT,
		G_IMPLEMENT_INTERFACE(ROCCAT_EVENTHANDLER_PLUGIN_TYPE, koneaimo_roccat_eventhandler_interface_init));

static void koneaimo_eventhandler_init(KoneaimoEventhandler *eventhandler) {
	eventhandler->priv = KONEAIMO_EVENTHANDLER_GET_PRIVATE(eventhandler);
}

static void dispose(GObject *object) {
	koneaimo_eventhandler_stop(ROCCAT_EVENTHANDLER_PLUGIN(object));

	G_OBJECT_CLASS(koneaimo_eventhandler_parent_class)->dispose(object);
}

static void finalize(GObject *object) {
	KoneaimoEventhandlerPrivate *priv = KONEAIMO_EVENTHANDLER(object)->priv;
	...
	G_OBJECT_CLASS(koneaimo_eventhandler_parent_class)->finalize(object);
}

static void koneaimo_eventhandler_class_init(KoneaimoEventhandlerClass *klass) {
	GObjectClass *gobject_class = G_OBJECT_CLASS(klass);
	gobject_class->dispose = dispose;
	gobject_class->finalize = finalize;

	g_type_class_add_private(klass, sizeof(KoneaimoEventhandlerPrivate));
}

ROCCAT_EVENTHANDLER_PLUGIN_API RoccatEventhandlerPlugin *roccat_eventhandler_plugin_new(void) {
	return ROCCAT_EVENTHANDLER_PLUGIN(g_object_new(KONEAIMO_EVENTHANDLER_TYPE, NULL));
}
