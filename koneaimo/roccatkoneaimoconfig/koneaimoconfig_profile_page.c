...
#include "koneaimo_color_frame.h"
...
#define KONEAIMOCONFIG_PROFILE_PAGE_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), KONEAIMOCONFIG_PROFILE_PAGE_TYPE, KoneaimoconfigProfilePageClass))
#define IS_KONEAIMOCONFIG_PROFILE_PAGE_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), KONEAIMOCONFIG_PROFILE_PAGE_TYPE))
#define KONEAIMOCONFIG_PROFILE_PAGE_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), KONEAIMOCONFIG_PROFILE_PAGE_TYPE, KoneaimoconfigProfilePagePrivate))

typedef struct _KoneaimoconfigProfilePageClass KoneaimoconfigProfilePageClass;
typedef struct _KoneaimoconfigProfilePagePrivate KoneaimoconfigProfilePagePrivate;

struct _KoneaimoconfigProfilePage {
	RoccatProfilePage parent;
	KoneaimoconfigProfilePagePrivate *priv;
};

struct _KoneaimoconfigProfilePageClass {
	RoccatProfilePageClass parent_class;
};

struct _KoneaimoconfigProfilePagePrivate {
	KoneplusRmp *rmp;
	KoneplusButtonsFrame *buttons;
	RoccatGamefileSelector *gamefiles;
	RoccatPollingRateFrame *polling_rate;
	KoneaimoCpiSelector *cpi;
	RoccatSensitivityFrame *sensitivity;
	KoneplusLightEffectsFrame *light_effects;
	KoneaimoColorFrame *colors;
	RoccatTalkFXSelector *talkfx;
};

G_DEFINE_TYPE(KoneaimoconfigProfilePage, koneaimoconfig_profile_page, ROCCAT_PROFILE_PAGE_TYPE);

GtkWidget *koneaimoconfig_profile_page_new(void) {
	return GTK_WIDGET(g_object_new(KONEAIMOCONFIG_PROFILE_PAGE_TYPE, NULL));
}

static void set_from_rmp(KoneaimoconfigProfilePage *profile_page) {
	KoneaimoconfigProfilePagePrivate *priv = profile_page->priv;
	guint i;
	...
	koneaimo_color_frame_set_from_rmp(priv->colors, priv->rmp);
	koneaimo_cpi_selector_set_from_rmp(priv->cpi, priv->rmp);
...
static void update_rmp(KoneaimoconfigProfilePage *profile_page, KoneplusRmp *rmp) {
	KoneaimoconfigProfilePagePrivate *priv = profile_page->priv;
	gchar *profile_name;
	guint i;
	...
	koneaimo_color_frame_update_rmp(priv->colors, rmp);
	koneaimo_cpi_selector_update_rmp(priv->cpi, rmp);
...
static void reset_cb(RoccatProfilePage *profile_page, gpointer user_data) {
	set_from_rmp(KONEAIMOCONFIG_PROFILE_PAGE(profile_page));
}

static void koneaimoconfig_profile_page_init(KoneaimoconfigProfilePage *profile_page) {
	KoneaimoconfigProfilePagePrivate *priv = KONEAIMOCONFIG_PROFILE_PAGE_GET_PRIVATE(profile_page);
	GtkWidget *content;
	GtkWidget *left;
	GtkWidget *light;
	GtkWidget *color;
	...
	priv->cpi = KONEAIMO_CPI_SELECTOR(koneaimo_cpi_selector_new());
	...
	priv->colors = KONEAIMO_COLOR_FRAME(koneaimo_color_frame_new());
...
static void koneaimoconfig_profile_page_class_init(KoneaimoconfigProfilePageClass *klass) {
	g_type_class_add_private(klass, sizeof(KoneaimoconfigProfilePagePrivate));
}
