...
#include "koneaimoconfig_window.h"
#include "koneaimoconfig_configuration_dialog.h"
#include "koneaimoconfig_profile_page.h"
...
#define KONEAIMOCONFIG_WINDOW_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), KONEAIMOCONFIG_WINDOW_TYPE, KoneaimoconfigWindowClass))
#define IS_KONEAIMOCONFIG_WINDOW_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), KONEAIMOCONFIG_WINDOW_TYPE))
#define KONEAIMOCONFIG_WINDOW_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), KONEAIMOCONFIG_WINDOW_TYPE, KoneaimoconfigWindowPrivate))

typedef struct _KoneaimoconfigWindowClass KoneaimoconfigWindowClass;
typedef struct _KoneaimoconfigWindowPrivate KoneaimoconfigWindowPrivate;

struct _KoneaimoconfigWindow {
	RoccatConfigWindowPages parent;
	KoneaimoconfigWindowPrivate *priv;
};

struct _KoneaimoconfigWindowClass {
	RoccatConfigWindowPagesClass parent_class;
};

struct _KoneaimoconfigWindowPrivate {
	RoccatKeyFile *config;
	DBusGProxy *dbus_proxy;
};

G_DEFINE_TYPE(KoneaimoconfigWindow, koneaimoconfig_window, ROCCAT_CONFIG_WINDOW_PAGES_TYPE);
...
static gboolean change_profile_handler(gpointer user_data) {
	...
	if (!roccat_handle_error_dialog(GTK_WINDOW(roccat_window), _("Could not activate profile"), &local_error))
		return FALSE;
	koneaimo_dbus_emit_profile_changed_outside(KONEAIMOCONFIG_WINDOW(roccat_window)->priv->dbus_proxy, profile_number);
...
static void device_add_cb(RoccatConfigWindow *roccat_window, gpointer user_data) {
	KoneaimoconfigWindow *window = KONEAIMOCONFIG_WINDOW(roccat_window);
	KoneaimoconfigWindowPrivate *priv = window->priv;
	GError *local_error = NULL;
	guint i;
	KoneplusRmp *rmp;
	RoccatDevice *device;
	KoneaimoconfigProfilePage *profile_page;
	guint actual_profile_index;

	device = roccat_config_window_get_device(roccat_window);

	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		profile_page = KONEAIMOCONFIG_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(roccat_window), i));
		rmp = koneaimoconfig_profile_page_get_rmp(profile_page);
		koneaimo_rmp_update_from_device(rmp, device, i);
		koneaimoconfig_profile_page_set_rmp(profile_page, rmp);
...
static KoneplusRmp *get_rmp(KoneaimoconfigWindow *window, KoneaimoconfigProfilePage *profile_page) {
	KoneplusRmp *rmp;
	gboolean moved;
...
static void set_rmp(KoneaimoconfigWindow *window, guint profile_index, KoneplusRmp *rmp) {
	KoneaimoconfigProfilePage *profile_page = KONEAIMOCONFIG_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(window), profile_index));
	guint profile_number = profile_index + 1;
	koneaimoconfig_profile_page_set_rmp(profile_page, rmp);
	roccat_config_window_pages_set_page_unmoved(ROCCAT_CONFIG_WINDOW_PAGES(window), ROCCAT_PROFILE_PAGE(profile_page));
	koneaimo_dbus_emit_profile_data_changed_outside(window->priv->dbus_proxy, profile_number);
}

static gboolean save_single(KoneaimoconfigWindow *window, KoneplusRmp *rmp, guint profile_index, GError **error) {
	RoccatDevice *device = roccat_config_window_get_device(ROCCAT_CONFIG_WINDOW(window));

	if (device)
		koneaimo_rmp_save(device, rmp, profile_index, error);
	else
...
static gboolean save_all(KoneaimoconfigWindow *window, gboolean ask) {
	...
	modified = FALSE;
	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		profile_page = KONEAIMOCONFIG_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(window), i));
...
static gboolean delete_event_cb(GtkWidget *window, GdkEvent *event, gpointer user_data) {
	return !save_all(KONEAIMOCONFIG_WINDOW(window), TRUE);
}
...
static gchar *koneaimo_create_filename_proposition(KoneplusRmp *rmp, guint profile_index) {
	gchar *profile_name;
	gchar *filename;
...
static void save_profile_to_file_cb(RoccatProfilePage *profile_page, gpointer user_data) {
	KoneaimoconfigWindow *window = KONEAIMOCONFIG_WINDOW(user_data);
	KoneaimoconfigWindowPrivate *priv = window->priv;
	GtkWidget *dialog;
	gchar *filename, *path;
	gchar *temp_filename;
	...
	rmp = koneaimoconfig_profile_page_get_rmp(KONEAIMOCONFIG_PROFILE_PAGE(profile_page));
	...
	filename = koneaimo_create_filename_proposition(rmp, profile_index);
...
static void load_profile_from_file_cb(RoccatProfilePage *profile_page, gpointer user_data) {
	...
	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
		...
		rmp = koneplus_rmp_read_with_path(filename, koneaimo_rmp_defaults(), &error);
		if (error) {
			roccat_warning_dialog(GTK_WINDOW(dialog), _("Could not load profile"), error->message);
			g_error_free(error);
		} else {
			koneplus_rmp_set_modified(rmp);
			koneaimoconfig_profile_page_set_rmp(KONEAIMOCONFIG_PROFILE_PAGE(profile_page), rmp);
			koneplus_rmp_free(rmp);
		}
...
static void save_all_cb(RoccatConfigWindow *roccat_window, gpointer user_data) {
	save_all(KONEAIMOCONFIG_WINDOW(roccat_window), FALSE);
}
...
static void menu_edit_preferences_cb(GtkMenuItem *item, gpointer user_data) {
	KoneaimoconfigWindow *window = KONEAIMOCONFIG_WINDOW(user_data);
	KoneaimoconfigWindowPrivate *priv = window->priv;
	koneaimoconfig_configuration_dialog(GTK_WINDOW(window), priv->config);
	koneaimo_dbus_emit_configuration_changed_outside(priv->dbus_proxy);
}
...
static void menu_device_reset_cb(GtkMenuItem *item, gpointer user_data) {
	...
	for (i = 0; i < KONEPLUS_PROFILE_NUM; ++i) {
		profile_page = KONEAIMOCONFIG_PROFILE_PAGE(roccat_config_window_pages_get_page(ROCCAT_CONFIG_WINDOW_PAGES(window), i));
		rmp = koneaimo_rmp_load_save_after_reset(device, i, &error);
		koneaimoconfig_profile_page_set_rmp(profile_page, rmp);
		koneplus_rmp_free(rmp);
		koneaimo_dbus_emit_profile_data_changed_outside(priv->dbus_proxy, i + 1);
	}
}

GtkWidget *koneaimoconfig_window_new(void) {
	return GTK_WIDGET(g_object_new(KONEAIMOCONFIG_WINDOW_TYPE,
			"device-name", KONEAIMO_DEVICE_NAME,
			NULL));
}

static GObject *constructor(GType gtype, guint n_properties, GObjectConstructParam *properties) {
	GObject *obj;
	KoneaimoconfigWindow *window;
	KoneaimoconfigWindowPrivate *priv;
	...
	obj = G_OBJECT_CLASS(koneaimoconfig_window_parent_class)->constructor(gtype, n_properties, properties);
	window = KONEAIMOCONFIG_WINDOW(obj);
	priv = window->priv;
	...
	/* keep this order */
	priv->dbus_proxy = koneaimo_dbus_proxy_new();

	roccat_config_window_set_device_scanner(roccat_window, ROCCAT_DEVICE_SCANNER_INTERFACE(koneaimo_device_scanner_new()));
...
static void koneaimoconfig_window_init(KoneaimoconfigWindow *window) {
	window->priv = KONEAIMOCONFIG_WINDOW_GET_PRIVATE(window);
}

static void finalize(GObject *object) {
	KoneaimoconfigWindowPrivate *priv = KONEAIMOCONFIG_WINDOW(object)->priv;

	g_clear_pointer(&priv->dbus_proxy, dbus_roccat_proxy_free);
	koneaimoconfig_configuration_save(GTK_WINDOW(object), priv->config);
	g_clear_pointer(&priv->config, koneplus_configuration_free);

	G_OBJECT_CLASS(koneaimoconfig_window_parent_class)->finalize(object);
}

static void koneaimoconfig_window_class_init(KoneaimoconfigWindowClass *klass) {
	GObjectClass *gobject_class = G_OBJECT_CLASS(klass);

	gobject_class->constructor = constructor;
	gobject_class->finalize = finalize;

	g_type_class_add_private(klass, sizeof(KoneaimoconfigWindowClass));
}
