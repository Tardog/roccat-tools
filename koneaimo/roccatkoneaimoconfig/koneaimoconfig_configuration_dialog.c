...
#define KONEAIMOCONFIG_CONFIGURATION_DIALOG_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), KONEAIMOCONFIG_CONFIGURATION_DIALOG_TYPE, KoneaimoconfigConfigurationDialogClass))
#define IS_KONEAIMOCONFIG_CONFIGURATION_DIALOG_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), KONEAIMOCONFIG_CONFIGURATION_DIALOG_TYPE))
#define KONEAIMOCONFIG_CONFIGURATION_DIALOG_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), KONEAIMOCONFIG_CONFIGURATION_DIALOG_TYPE, KoneaimoconfigConfigurationDialogPrivate))

typedef struct _KoneaimoconfigConfigurationDialogClass KoneaimoconfigConfigurationDialogClass;

struct _KoneaimoconfigConfigurationDialogClass {
	GtkDialogClass parent_class;
};

struct _KoneaimoconfigConfigurationDialogPrivate {
...
G_DEFINE_TYPE(KoneaimoconfigConfigurationDialog, koneaimoconfig_configuration_dialog, GTK_TYPE_DIALOG);

static GtkWidget *notification_frame_new(KoneaimoconfigConfigurationDialog *koneaimoconfig_configuration_dialog) {
	KoneaimoconfigConfigurationDialogPrivate *priv = koneaimoconfig_configuration_dialog->priv;
...
static GtkWidget *general_frame_new(KoneaimoconfigConfigurationDialog *koneaimoconfig_configuration_dialog) {
	KoneaimoconfigConfigurationDialogPrivate *priv = koneaimoconfig_configuration_dialog->priv;
...
gboolean koneaimoconfig_configuration_dialog(GtkWindow *parent, RoccatKeyFile *config) {
	KoneaimoconfigConfigurationDialog *dialog;
	gboolean retval = TRUE;

	dialog = KONEAIMOCONFIG_CONFIGURATION_DIALOG(koneaimoconfig_configuration_dialog_new(parent));

	koneaimoconfig_configuration_dialog_set_configuration(dialog, config);

	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
		koneaimoconfig_configuration_dialog_get_configuration(dialog, config);
		if (!koneaimoconfig_configuration_save(parent, config))
			retval = FALSE;
	}
...
GtkWidget *koneaimoconfig_configuration_dialog_new(GtkWindow *parent) {
	KoneaimoconfigConfigurationDialog *koneaimoconfig_configuration_dialog;

	koneaimoconfig_configuration_dialog = g_object_new(KONEAIMOCONFIG_CONFIGURATION_DIALOG_TYPE, NULL);

	gtk_dialog_add_buttons(GTK_DIALOG(koneaimoconfig_configuration_dialog),
			GTK_STOCK_OK, GTK_RESPONSE_ACCEPT,
			GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL,
			NULL);
	gtk_window_set_title(GTK_WINDOW(koneaimoconfig_configuration_dialog), _("Edit preferences"));
	gtk_window_set_transient_for(GTK_WINDOW(koneaimoconfig_configuration_dialog), parent);
	gtk_window_set_modal(GTK_WINDOW(koneaimoconfig_configuration_dialog), TRUE);
	gtk_window_set_destroy_with_parent(GTK_WINDOW(koneaimoconfig_configuration_dialog), TRUE);
	gtk_window_set_type_hint(GTK_WINDOW(koneaimoconfig_configuration_dialog), GDK_WINDOW_TYPE_HINT_NORMAL);

	return GTK_WIDGET(koneaimoconfig_configuration_dialog);
}

static void koneaimoconfig_configuration_dialog_init(KoneaimoconfigConfigurationDialog *koneaimoconfig_configuration_dialog) {
	KoneaimoconfigConfigurationDialogPrivate *priv = KONEAIMOCONFIG_CONFIGURATION_DIALOG_GET_PRIVATE(koneaimoconfig_configuration_dialog);
	GtkVBox *content_area;

	koneaimoconfig_configuration_dialog->priv = priv;

	content_area = GTK_VBOX(gtk_dialog_get_content_area(GTK_DIALOG(koneaimoconfig_configuration_dialog)));

	gtk_box_pack_start(GTK_BOX(content_area), notification_frame_new(koneaimoconfig_configuration_dialog), TRUE, TRUE, 0);
	gtk_box_pack_start(GTK_BOX(content_area), general_frame_new(koneaimoconfig_configuration_dialog), TRUE, TRUE, 0);

	gtk_widget_show_all(GTK_WIDGET(content_area));
}

static void koneaimoconfig_configuration_dialog_class_init(KoneaimoconfigConfigurationDialogClass *klass) {
	g_type_class_add_private(klass, sizeof(KoneaimoconfigConfigurationDialogPrivate));
}

gboolean koneaimoconfig_configuration_save(GtkWindow *parent, RoccatKeyFile *config) {
	GError *error = NULL;

	koneaimo_configuration_save(config, &error);
