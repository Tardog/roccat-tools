...
KoneplusProfileSettings *koneaimo_rmp_to_profile_settings(KoneplusRmp *rmp) {
	...
	for (i = 0; i < KONEPLUS_PROFILE_SETTING_CPI_LEVELS_NUM; ++i) {
		settings->cpi_levels_x[i] = koneaimo_rmp_cpi_to_bin(koneplus_rmp_get_cpi_level_x(rmp, i));
		settings->cpi_levels_y[i] = koneaimo_rmp_cpi_to_bin(koneplus_rmp_get_cpi_level_y(rmp, i));
	}
	...
	for (i = 0; i < KONEPLUS_PROFILE_SETTING_LIGHTS_NUM; ++i) {
		...
		if (koneaimo_rmp_get_light_chose_type(rmp) == KONEXTD_RMP_LIGHT_CHOSE_TYPE_TABLE) {
			...
			rmp_light_info = koneaimo_rmp_get_custom_light_info(rmp, i);
...
void koneaimo_rmp_update_with_profile_settings(KoneplusRmp *rmp, KoneplusProfileSettings const *profile_settings) {
	...
	for (i = 0; i < KONEPLUS_PROFILE_SETTING_CPI_LEVELS_NUM; ++i) {
		koneplus_rmp_set_cpi_level_x(rmp, i, koneaimo_bin_cpi_to_rmp(profile_settings->cpi_levels_x[i]));
		koneplus_rmp_set_cpi_level_y(rmp, i, koneaimo_bin_cpi_to_rmp(profile_settings->cpi_levels_y[i]));
	}
	...
	for (i = 0; i < KONEPLUS_PROFILE_SETTING_LIGHTS_NUM; ++i) {
		if (profile_settings->lights[0].index == KONEPLUS_LIGHT_INFO_INDEX_CUSTOM) {
			...
			koneaimo_rmp_set_custom_light_info(rmp, i, &rmp_light_info);
