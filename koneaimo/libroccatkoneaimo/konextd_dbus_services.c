...
#include "roccat.h"
#include "koneaimo.h"
#include "koneaimo_dbus_plain_services.h"
...
DBusGProxy *koneaimo_dbus_proxy_new(void) {
	...
	proxy = dbus_roccat_proxy_new_for_name(ROCCAT_DBUS_SERVER_NAME,
			KONEAIMO_DBUS_SERVER_PATH,
			KONEAIMO_DBUS_SERVER_INTERFACE);
...
gboolean koneaimo_dbus_emit_profile_data_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Koneaimo_profile_data_changed_outside, profile_number);
}
...
gboolean koneaimo_dbus_emit_profile_data_changed_outside_instant(guint profile_number) {
	...
	retval = koneaimo_dbus_emit_profile_data_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean koneaimo_dbus_emit_profile_changed_outside(DBusGProxy *proxy, guint profile_number) {
	return dbus_roccat_call_y(proxy, org_roccat_Koneaimo_profile_changed_outside, profile_number);
}
...
gboolean koneaimo_dbus_emit_profile_changed_outside_instant(guint profile_number) {
	...
	retval = koneaimo_dbus_emit_profile_changed_outside(proxy, profile_number);
	dbus_roccat_proxy_free(proxy);
	return retval;
}

gboolean koneaimo_dbus_emit_configuration_changed_outside(DBusGProxy *proxy) {
	return dbus_roccat_call(proxy, org_roccat_Koneaimo_configuration_changed_outside);
}
