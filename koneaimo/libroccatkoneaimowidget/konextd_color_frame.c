...
#define KONEAIMO_COLOR_FRAME_CLASS(klass) (G_TYPE_CHECK_CLASS_CAST((klass), KONEAIMO_COLOR_FRAME_TYPE, KoneaimoColorFrameClass))
#define IS_KONEAIMO_COLOR_FRAME_CLASS(klass) (G_TYPE_CHECK_CLASS_TYPE((klass), KONEAIMO_COLOR_FRAME_TYPE))
#define KONEAIMO_COLOR_FRAME_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), KONEAIMO_COLOR_FRAME_TYPE, KoneaimoColorFramePrivate))

typedef struct _KoneaimoColorFrameClass KoneaimoColorFrameClass;
typedef struct _KoneaimoColorFramePrivate KoneaimoColorFramePrivate;

struct _KoneaimoColorFrame {
	GtkFrame parent;
	KoneaimoColorFramePrivate *priv;
};

struct _KoneaimoColorFrameClass {
	GtkFrameClass parent_class;
};

struct _KoneaimoColorFramePrivate {
...
G_DEFINE_TYPE(KoneaimoColorFrame, koneaimo_color_frame, GTK_TYPE_FRAME);
...
static void update(KoneaimoColorFrame *frame) {
	KoneaimoColorFramePrivate *priv = frame->priv;
...
void koneaimo_color_frame_set_from_rmp(KoneaimoColorFrame *frame, KoneplusRmp *rmp) {
	KoneaimoColorFramePrivate *priv = frame->priv;
	...
	for (i = 0; i < KONEPLUS_PROFILE_SETTING_LIGHTS_NUM; ++i) {
		...
		light_info = koneaimo_rmp_get_custom_light_info(rmp, i);
...
void koneaimo_color_frame_update_rmp(KoneaimoColorFrame *frame, KoneplusRmp *rmp) {
	KoneaimoColorFramePrivate *priv = frame->priv;
	KoneplusRmpLightInfo light_info;
	...
	koneaimo_rmp_set_light_chose_type(rmp, gtk_toggle_button_get_active(priv->use_palette) ? KONEAIMO_RMP_LIGHT_CHOSE_TYPE_TABLE : KONEAIMO_RMP_LIGHT_CHOSE_TYPE_CUSTOM);
	...
	for (i = 0; i < KONEPLUS_PROFILE_SETTING_LIGHTS_NUM; ++i) {
		...
		koneaimo_rmp_set_custom_light_info(rmp, i, &light_info);
	}
}

static void use_palette_cb(GtkToggleButton *toggle_button, gpointer user_data) {
	update(KONEAIMO_COLOR_FRAME(user_data));
}

GtkWidget *koneaimo_color_frame_new(void) {
	return GTK_WIDGET(g_object_new(KONEAIMO_COLOR_FRAME_TYPE, NULL));
}

static void koneaimo_color_frame_init(KoneaimoColorFrame *frame) {
	KoneaimoColorFramePrivate *priv = KONEAIMO_COLOR_FRAME_GET_PRIVATE(frame);
	GtkWidget *hbox;
	GtkWidget *table;
	GtkWidget *vbox;
	guint i;
...
static void koneaimo_color_frame_class_init(KoneaimoColorFrameClass *klass) {
	g_type_class_add_private(klass, sizeof(KoneaimoColorFramePrivate));
}
